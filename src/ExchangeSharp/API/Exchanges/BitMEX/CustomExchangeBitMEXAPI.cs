namespace ExchangeSharp
{
	...
	public sealed partial class ExchangeBitMEXAPI : ExchangeAPI
	{
		protected override async Task<ExchangeTicker> OnGetTickerAsync(string marketSymbol)
		{
			...
			var query = $"/instrument?symbol={marketSymbol}";
			JToken token = await MakeJsonRequestAsync<JToken>(query, BaseUrl, payload, "GET");

			var firstSymbol = token[0];
			var bid = firstSymbol["bidPrice"].ConvertInvariant<decimal>();
			var ask = firstSymbol["askPrice"].ConvertInvariant<decimal>();
			var last = firstSymbol["lastPrice"].ConvertInvariant<decimal>();
